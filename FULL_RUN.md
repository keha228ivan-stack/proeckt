...
## 0.1) Проверка каталогов клиентов в этом репозитории

На **2026-05-05** каталоги клиентов подключены как gitlink (submodule pointers) без локально инициализированного содержимого. Поэтому правки клиентского кода нужно выполнять в соответствующих репозиториях клиентов после их инициализации/клонирования.

Быстрая проверка:
- `git ls-tree HEAD` показывает тип `160000 commit` для `hrrepozik`, `hrrepozik-modile-2`, `repozik-desktop2`.
- В текущем репозитории эти каталоги не содержат версионируемых файлов для прямого редактирования.
